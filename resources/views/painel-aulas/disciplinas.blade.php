            ...
            @foreach ($categories as $category)
                <div>
                    <div class="flex items-center text-2xl font-bold mb-2.5">
                        <span class="text-3xl" style="color: {{ $category->color }}">|</span><h2>{{ $category->name }}</h2>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-5">
                        @foreach ($category->disciplines as $discipline)
                            <a href="{{ route('topicos', $discipline) }}" style="background-color: {{ $category->color }}"
                                class="w-40 h-32 rounded-lg flex flex-col items-center justify-center shadow-md text-center cursor-pointer transition-all duration-300 hover:-translate-y-1.5 hover:brightness-75">
