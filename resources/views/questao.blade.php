                        ...
                        @foreach ($question->answers as $index => $answer)
                            <li>
                                <label for="option_{{ $question->id }}_{{ $answer->id }}" class="flex items-center">
                                    <input type="radio" id="option_{{ $question->id }}_{{ $answer->id }}"
                                        name="options_{{ $question->id }}" value="{{ $answer->id }}" class="mr-2">
                                    <span class="text-gray-700 dark:text-white">({{ chr(65 + $index) }})
